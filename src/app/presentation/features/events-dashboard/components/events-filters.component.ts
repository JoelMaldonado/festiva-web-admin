...
import { AppDateRangeComponent } from '@components/app-date-range.component';
      ...
      >
        ...
        <div class="relative p-5 md:p-6">
          ...
          <div class="flex items-center justify-between gap-3">
            <div>
              <h2 class="text-lg md:text-xl font-extrabold text-white">Gestiona tus eventos</h2>
              <p class="text-sm text-slate-400">Crea, busca y filtra eventos del club.</p>
            </div>
          ...
          <div class="mt-4">
            <fest-input
              label="Search"
              placeholder="Buscar eventos por nombre (busca en todas las fechas)"
          ...
          <!-- Categorías -->
          <p class="text-sm text-slate-400 mt-4 mb-2">Categorías</p>
          <div class="flex flex-wrap gap-2 gap-y-3">
            @for (item of service.listCategory(); track $index) {
              <button
                ...
                [ngClass]="service.selectedCategory() === item
          ...
          <div class="mt-4">
            <p class="text-sm text-slate-400 mb-2">
              Rango de fechas
              @if (service.searchInput()) {
                <span class="ml-2 text-xs text-white/30 italic">(ignorado durante la búsqueda)</span>
              }
            </p>
            <app-date-range
              (rangeChange)="service.setDateRange($event.from, $event.to)"
            />
          </div>
